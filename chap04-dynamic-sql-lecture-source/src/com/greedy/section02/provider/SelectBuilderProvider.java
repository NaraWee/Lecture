...
public class SelectBuilderProvider {
	...
	/* SQL문을 문자열 형태로 반환하도록 반환타입을 지정 */
	public String selectAllMenu() {
		
		/* 
		 * static 메소드 형태로 SELECT() 이런 메소드가 있었는데 deprecated 되어있다.
		 * 최근에는 Builder 패턴을 이용한 형태로 사용하고 있다.
		 * 3.2버전 이전에는 다른 방법을 사용했다.
		 * 최근에는 빌더 타입의 패턴과 익명 내부 클래스를 사용하는 추세이다. SelectBuilder와 SqlBuilder는 향후 제거 할 예정이다(공식 문서에서 발췌함)
		 */
		return new SQL()
				.SELECT("A.MENU_CODE")
				.SELECT("A.MENU_NAME")
	...
	public String searchMenuByCondition(SearchCriteria searchCriteria) {
		...
		if("category".equals(searchCriteria.getCondition())) {
			...
			/* 가변인자를 이용하면 자동 AND로 처리하기 때문에 OR를 사용해야 하는 경우 .OR로 이용해야한다. */
